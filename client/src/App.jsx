                ...
                <div key={column} className="w-80 flex-shrink-0 flex flex-col">
                  ...
                  <div className="flex-1 bg-slate-100/30 rounded-[2.5rem] p-4 space-y-4 overflow-y-auto border-2 border-dashed border-slate-200/50">
                    {tasks.filter(t => t.status === column).map(task => (
                      <div key={task._id} onClick={() => updateTaskStatus(task._id, task.status)} className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 hover:border-indigo-400 hover:shadow-xl hover:-translate-y-1 transition-all cursor-pointer group relative overflow-hidden">
                        <div className="flex justify-between items-start mb-4">
